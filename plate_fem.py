...
def plot_element_solution(element, edge_conn, elem_edge_conn, X, soln, fix, ax, npts=1):
    # Get the node coordinates for the element.
    n1_coord, n2_coord, n3_coord = get_node_coords(
        element, edge_conn, elem_edge_conn, X
    )
...
def plot_mesh(X, elem_conn, edge_conn, elem_edge_conn):
    ...
    plt.savefig("plate_mesh.jpg", dpi=1000)
    return
...
# plot_mesh(X, elem_conn, edge_node_conn, elem_edge_conn)
...
shared_edges = get_shared_edge_tags(edge_node_conn)
...
for e in range(len(elem_conn)):
    plot_element_solution(
        e, edge_node_conn, elem_edge_conn, X, u[elem_edge_conn[e]], fig1, ax1, npts=1
    )
